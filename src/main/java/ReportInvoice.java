...
public class ReportInvoice {
	...
	public static void searchById(String user, String pass, String url) throws IOException, InterruptedException {
		...
		try {
			...
			int count = 0;
			String sql = "select * from CUSTOMER_INVOICE INNER JOIN  ITEMS ON ITEMS.Item_Id=CUSTOMER_INVOICE.Item_Id INNER JOIN Shop ON Shop.Shop_Id=ITEMS.Shop_Id INNER JOIN InvoiceHeader ON InvoiceHeader.Header_id=Shop.Header_id where Customer_Id='"
					+ InvoiceNO + "'";
			ResultSet rs = st.executeQuery(sql);
